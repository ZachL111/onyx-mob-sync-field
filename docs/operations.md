...
`onyx-mob-sync-field` is centered on mobile workflows work in Go. The technical target is: Create a Go reference implementation for sync workflows, centered on graph analysis, node-edge fixtures, and cycle and reachability reports.
...
The score is deterministic. The current threshold is 156. Risk has a penalty of 4, latency has a penalty of 4, and weight contributes a bonus of 3.
